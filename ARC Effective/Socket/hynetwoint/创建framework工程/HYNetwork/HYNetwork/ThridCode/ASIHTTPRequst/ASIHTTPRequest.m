...
//            NSLog(@"_________%llu",self.postLength);
		}
...
- (void)startRequest
{
   ...
   // Start the HTTP connection
	...
	[self setStatusTimer:[NSTimer timerWithTimeInterval:0.5 target:self selector:@selector(updateStatus:) userInfo:nil repeats:YES]];
...
- (BOOL)shouldTimeOut
{
	...
	if ([self readStream] && [self readStreamIsScheduled] && [self lastActivityTime] && [self timeOutSeconds] > 0 && secondsSinceLastActivity > [self timeOutSeconds]) {
		...
		} else if (secondsSinceLastActivity > [self timeOutSeconds]*1) {
			return YES;
		}
	}
	return NO;
...
{
    if([[self.responseHeaders valueForKey:@"Content-Length"] integerValue]>0){
        ...
        //本次发送多少b，总共发送了多少b，总共多少b
#if DEBUGOUT
        LOG(@"  本次下载%llu,总共下载%llu,总共%@",value,bytesReadSoFar, [self.responseHeaders valueForKey:@"Content-Length"]);
#endif
        
        CGFloat zeroRate=(CGFloat)bytesSentZeroNum/(CGFloat)bytesSentNum;
        if(bytesSentNum/2>30 && zeroRate>0.5 && bytesReadSoFar/bytesSentNum/2.0/1024.0<10.0){
#if DEBUGOUT
            LOG(@"  bytesSentNum/2>30 && zeroRate>0.5(请求超过30s，并且接收0b的次数超过一半)  平均速度%.2f kb",bytesReadSoFar/bytesSentNum/2.0/1024.0);
#endif
            [self cancel];
        }
...
- (void)updateUploadProgress
{
    if([self postLength]>0){
        ...
        //本次发送多少b，总共发送了多少b，总共多少b
#if DEBUGOUT
        LOG(@"  本次发送%llu,总共发送%llu,总共%llu",value,[self totalBytesSent], [self postLength]);
#endif
        CGFloat zeroRate=(CGFloat)bytesSentZeroNum/(CGFloat)bytesSentNum;
        if(bytesSentNum/2>30 && zeroRate>0.5 && [self totalBytesSent]/bytesSentNum/2.0/1024.0<10.0){
#if DEBUGOUT
            LOG(@"  bytesSentNum/2>30 && zeroRate>0.5(请求超过30s，并且发送0b的次数超过一半)  平均速度%.2f kb",[self totalBytesSent]/bytesSentNum/2.0/1024.0);
#endif
            [self cancel];
        }
...
@synthesize bytesSentNum;
@synthesize bytesSentZeroNum;
