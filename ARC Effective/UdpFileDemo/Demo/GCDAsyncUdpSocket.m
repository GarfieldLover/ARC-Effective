	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if (af == AF_INET)
					result = sendto((int)socket4FD, (const void *)buffer, (size_t)length, 
                                    (int)0, (const struct sockaddr *)dst, (socklen_t)dstSize);
				else
                    result = sendto((int)socket6FD, (const void *)buffer, (size_t)length, 
                                    (int)0, (const struct sockaddr *)dst, (socklen_t)dstSize);
